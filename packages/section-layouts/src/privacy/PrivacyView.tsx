...
/** Privacy notice body — used by live /privacy and Studio draft canvas. */
export function PrivacyView({
  hospitalName,
  title,
  intro: introRaw,
  formsNote: formsRaw,
  rightsNote: rightsRaw,
  homeHref,
  contactPhone,
  contactEmail,
  layoutVersion = 1,
  ...
  const defaultIntro = `This notice explains how ${hospitalName} handles personal information on this public marketing website, in line with India's Digital Personal Data Protection Act, 2023 (DPDP Act) principles. This site is not a patient portal and is not intended to collect clinical health records.`;
  ...
  const defaultRights = `Subject to applicable law, you may request access to, correction of, or erasure of personal data you provided through this website, and you may withdraw consent for further processing of form submissions. To exercise these rights, contact ${hospitalName} using the published contact details. You may also have the right to complain to the Data Protection Board of India once operational under the DPDP Act.`;

  const intro = introRaw.trim() || defaultIntro;
  const formsNote = formsRaw.trim() || defaultForms;
  const rightsNote = rightsRaw.trim() || defaultRights;
      ...
      {layout === 2 ? (
        ...
        >
          <div>
            ...
            <p>
              {hospitalName} is the organisation responsible for personal data collected through this
              website (for example appointment requests). Contact the hospital using the details
